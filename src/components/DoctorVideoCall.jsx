...
import { FaMicrophone, FaMicrophoneSlash, FaVideo, FaVideoSlash, FaPhoneSlash, FaExpand, FaCompress, FaComments, FaUserMd } from "react-icons/fa";
...
export default function DoctorVideoCall() {
    ...
    async function init() {
        try {
            ...
            // DOCTOR IS THE ANSWERER: Listens for the Offer from Patient
            socket.current.on("offer", async (data) => {
                ...
                try {
                    ...
                    socket.current.emit("answer", pc.current.localDescription, roomId);
                } catch (err) {
                    console.error("Error handling offer:", err);
                }
            });

            // Doctor ignores answers
            socket.current.on("answer", (data) => {
                console.warn("Ignoring unexpected answer, Doctor is the designated Answerer.", data);
            });
            ...
            socket.current.on("user-left", (socketId) => {
                console.log("Patient left the call:", socketId);
                setStatus("Patient Disconnected");
                setPatientConnected(false);
            });

            // No initial offer creation here.
        } catch (error) {
    ...
    return (
        <div className={`h-screen overflow-hidden bg-gradient-to-br from-green-900 via-teal-900 to-blue-900 flex flex-col ${isFullscreen ? 'fixed inset-0 z-50' : ''}`}>
            <AppointmentInfo appointmentId={appointmentId} userType="doctor" />
            {/* Header */}
            <div className="bg-black/30 backdrop-blur-md p-4 flex justify-between items-center border-b border-white/10">
                <div className="flex items-center space-x-4">
                    ...
                    <div className="flex items-center space-x-2">
                        ...
                        <span className="text-white font-medium">Doctor Room: {roomId}</span>
                    </div>
                ...
                <div className="flex items-center space-x-4">
                    {patientConnected && (
                        <div className="bg-green-500/20 border border-green-500/50 px-3 py-1 rounded-lg">
                            <span className="text-green-400 text-sm font-medium">Patient Connected</span>
                        </div>
                    )}
            ...
            <div className="flex-1 relative p-4 transition-all duration-300 flex justify-center min-h-0 overflow-hidden">
                    ...
                    {!patientConnected && (
                        <div className="absolute inset-0 flex items-center justify-center bg-black/50">
                            <div className="text-center">
                                <FaUserMd className="text-white/50 text-6xl mx-auto mb-4" />
                ...
                <div className="absolute bottom-24 left-4 w-56 h-40 rounded-xl overflow-hidden shadow-2xl border-2 border-white/20">
                    ...
                    <div className="absolute bottom-2 left-2 bg-black/70 backdrop-blur-sm px-2 py-1 rounded">
                        <span className="text-white text-xs">Dr. You</span>
                    </div>
                    {isMuted && (
